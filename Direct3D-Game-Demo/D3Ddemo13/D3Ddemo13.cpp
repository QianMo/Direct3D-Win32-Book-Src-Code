...
#define WINDOW_TITLE	_T("【致我们永不熄灭的游戏开发梦想】  水乳交融的艺术：alpha混合技术 示例程序") //为窗口标题定义的宏
...
void Matrix_Set()
{
	...
	D3DXVECTOR3 vEye(0.0f, 100.0f, -200.0f);  //摄像机的位置
	D3DXVECTOR3 vAt(0.0f, 35.0f, 0.0f); //观察点的位置
...
void Direct3D_Render(HWND hwnd)
{
	...
	for (DWORD i = 0; i < g_dwNumMtrls; i++)
	{
		...
		g_pMesh->DrawSubset(i);  //绘制此部分
			...
			formatRect.top += 35;
			g_pTextHelper->DrawText(NULL, L"    数字键1与2：增大或者缩小材质的Alpha值", -1, &formatRect, 

...
#define WINDOW_TITLE		L"【致我们永不熄灭的游戏开发梦想】愤怒的小鸟摩擦力环境模拟demo"	//为窗口标题定义的宏
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,LPSTR lpCmdLine, int nShowCmd)
{
	...
	PlaySound(L"NOIR2.wav", NULL, SND_FILENAME | SND_ASYNC|SND_LOOP); //循环播放背景音乐 
	...
	while( msg.message != WM_QUIT )		//使用while循环，如果消息不是WM_QUIT消息，就继续循环
	{
		...
		else
		{
			...
			if(g_tNow-g_tPre >= 40)        //当此次循环运行与上次绘图时间相差0.04秒时再进行重绘操作
				Game_Paint(hwnd);
		}
